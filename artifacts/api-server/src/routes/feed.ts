...
import { filmsTable, episodesTable, votePollsTable, voteOptionsTable, usersTable } from "@workspace/db";
...
router.get("/feed", async (req, res) => {
  ...
  const result = await Promise.all(episodes.map(async (episode, i) => {
    const [film] = await db.select().from(filmsTable).where(eq(filmsTable.id, episode.filmId));
...
router.get("/feed/trending", async (req, res) => {
  ...
  const pollsWithOptions = await Promise.all(hotPolls.map(async (poll) => {
    const options = await db.select().from(voteOptionsTable).where(eq(voteOptionsTable.pollId, poll.id));
    const total = options.reduce((sum, o) => sum + o.voteCount, 0) || 1;
    return {
      id: poll.id,
  ...
  const users = await db.select().from(usersTable).orderBy(desc(usersTable.xp)).limit(3);
...
router.get("/feed/stats", async (req, res) => {
  const [filmCount] = await db.select({ count: sql<number>`count(*)::int` }).from(filmsTable);
  const [episodeCount] = await db.select({ count: sql<number>`count(*)::int` }).from(episodesTable);
  const [activePollCount] = await db
    .select({ count: sql<number>`count(*)::int` })
    .from(votePollsTable)
    .where(eq(votePollsTable.isActive, true));
  const [totalVotes] = await db.select({ sum: sql<number>`coalesce(sum(total_votes), 0)::int` }).from(votePollsTable);
  const [userCount] = await db.select({ count: sql<number>`count(*)::int` }).from(usersTable);

  const genreRows = await db
    .select({ genre: filmsTable.genre, count: sql<number>`count(*)::int` })
    .from(filmsTable)
  ...
  res.json({
    ...
    topGenres: genreRows.map((r) => ({ genre: r.genre, count: r.count })),
  });
});

function serializeFilm(film: typeof filmsTable.$inferSelect) {
  return { ...film, createdAt: film.createdAt.toISOString() };
}
