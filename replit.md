...
*   **Cinematic VolumeControl:** Each FeedCard uses `components/feed/VolumeControl.tsx` (replaces a plain Mute/Unmute button). Tap toggles mute, hover (desktop) or long-press (touch) reveals a vertical slider with an amber gradient fill, knob, and live percentage badge; arrow keys fine-tune. Audible state shows a soft amber/rose glow ring and animated 3-bar equalizer. Mute + volume preference persists via the `useVolumePref` hook (`localStorage` key `cl:volume:v1`) and syncs across mounted cards through a `cl:volume` custom event so swiping between cards keeps your sound level. Volume changes are pushed to both HTML `<video>` (`videoRef.current.volume`) and YouTube iframes (`postMessage` `setVolume` 0–100).
*   **Pro Entitlements & Daily Quotas:** AI Director chat is rate-limited per UTC day with three tiers — 3/day anonymous, 5/day signed-in free, 100/day Pro. The limiter (`artifacts/api-server/src/lib/rate-limit.ts`) is an in-memory `Map` (single-server limitation; documented). Each chat response carries `x-ratelimit-limit/remaining/reset` headers; on 429 the API returns `{error: "rate_limited", upgrade: true, quota}` and the frontend surfaces it as a system turn with an inline "Upgrade to Pro" CTA linking `/pricing`. Pro users see a Crown badge in the Co-pilot dock; free users see a live "used/limit" counter that turns red at the cap. `/api/director/best-clip` enforces the same tier split — Free gets the heuristic-only ranking, Pro gets GPT-5.4 re-ranking, with separate cache keys (`:ai` / `:heur`) to prevent cross-tier bleed.
*   **Admin Console & Revenue Dashboard:** `/admin` renders revenue overview cards (lifetime + last-30-day, by provider), recent payments table, users table with one-click ban/unban, and an abuse events feed. Backed by `GET /api/admin/{overview,payments,users,abuse}` and `POST /api/admin/users/:id/ban` — all guarded by `requireAdmin` in `artifacts/api-server/src/lib/auth-guards.ts`. Admin status is determined by `users.is_admin` (DB flag) OR membership in the `ADMIN_EMAILS` env var; in dev, user `id=1` is treated as admin if neither is set. `/api/identity/me` exposes `isAdmin`, `isBanned`, and the live `chatQuota` snapshot. The Admin sidebar link only renders for admins.
*   **Abuse Tracking & Moderation:** New `abuse_events` table (`id`, `user_id` FK nullable, `ip`, `event_type`, `severity`, `metadata` JSON-string, `created_at` + indexes on `user_id` and `created_at`) records rate-limit overages, ban actions, and other suspicious events via `recordAbuse()` in `artifacts/api-server/src/lib/abuse.ts`. Banned users (`users.is_banned=true`) get a 403 `account_banned` from any auth-gated route. Self-ban is rejected with `cannot_ban_self`. Each ban/unban writes its own abuse event with the acting admin's id for audit.
